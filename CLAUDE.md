...
2. **Harvest Pass C first**: check `logs/passC2.log` for "PASS C COMPLETE" (passC was relaunched as passC2 after a machine reboot at 20:56 killed the first attempt at epoch ~8.4k — worklog 21:10); look at the sinusoidal `_x40` runs before anything else — they decide the headline wording ("blocks" vs "delays ≥Nx" grokking; DAY2_REPORT risk 1). If Pass C is still running, harvest what's done (`--skip-existing` relaunch if it crashed: `scripts/passC.ps1`).
