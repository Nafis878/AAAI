...
**D20 (S4, ~11:30). S-H3 grok-flag baseline already fires on CPU data — honest-null, reported not buried.** Wiring the pre-registered grok-flag baseline into `h3_ood.json` revealed that on the existing n=10 CPU OOD data the binary grokked-flag predicts OOD accuracy (LOO R²=0.986) slightly BETTER than the 6-metric signature ridge (R²=0.919): `signatures_beat_grok_flag=false`. This is the W3 circularity concern made concrete. It does NOT falsify any current paper claim (the paper compares signature vs PE label — signature still wins there, R² 0.92 vs −1.59 — and never claimed superiority over the grok flag). Per strengthening_plan §S-H3, the pre-committed honest framing applies: "signature ⊇ grok status; PE label ⊉ it." Likely partly an n=10 artifact (bimodal OOD acc makes the binary flag a near-perfect 2-group predictor while the 6-dim ridge slightly overfits under LOO); the GPU S-H3 grid (more seeds + alternative b-range OOD) is the proper test. Flagged in paper/RESULTS_TODO.md so the integration session applies the honest-null wording. No goalpost-moving: the pre-registration anticipated exactly this outcome.
